...
trusted.  The Reader and Action model outputs, retrieved email content,
...
the user still supplies the Alice recipient.\subsection{Defenses}
...
The transformation registry is append-only, but acceptance uses exact matching:
the source-value hash, output-value hash, transform name, and trusted source
authority must all match one registered entry.  The strengthened matrix uses the
semantically valid \texttt{NormalizeEmailAddress} transform.  A user-sourced
normalized address is accepted; an unregistered transform or an externally
claimed transform is denied.  This is provenance verification, not fuzzy string
matching.\section{Evaluation}

\subsection{Experimental Setup}

The primary artifact is \texttt{artifacts/pact-strengthened-v2/}.  It contains
...
SHA-256.  The eight strategy rows are deliberately decision-only and exercise
recipient, content, and control bindings plus registered, unregistered, and
externally claimed \texttt{NormalizeEmailAddress} metadata.

The reproducible entry point is
\texttt{scripts/}\texttt{run\_pact\_strengthened.py} with output directory
\texttt{artifacts/}\texttt{pact-strengthened-v2}.  The runner refuses to
...
stores the provenance digest and decision-log SHA-256 in the committed JSONL
artifact.
...
\texttt{artifacts/pact-strengthened-v2/}.  Its
\texttt{e2e\_results.csv/json} contain six real-tool records;
\texttt{strategy\_results.csv/json} contain eight decision-only rows;
\texttt{decision\_log.jsonl} contains canonical structured decisions;
...
\texttt{pact-strengthened-v2} directory and refuses to overwrite an existing
artifact.  The artifact is bound to commit
\texttt{2aaad60c}.  The full commit is recorded in the manifest.  The earlier
...
\path{artifacts/pact-strengthened-v2/}.  The six real-tool records are in
