...
Project repository:
\href{https://github.com/Chikit-WONG/provenance-aware-capability-gating}
{\textcolor{blue}{\texttt{github.com/Chikit-WONG/provenance-aware-capability-gating}}}}
\date{Summer 2026}
...
% Previous architecture description and figure retained for revision history.
...
The Reader/Action split exposes cross-step authority laundering: untrusted
email may influence structured arguments while event-linked provenance flows to
the Action Agent. Figure~\ref{fig:architecture} shows the gateway checking
capabilities, argument roles, source authority, exact registered transformations,
and sinks before \texttt{ToolExecutor}. \textsc{Allow} reaches MockWorld;
\textsc{Deny} is logged without tool execution or side effects. Evaluation uses
decision logs and world state, not transcript keywords.

\begin{figure*}[t]
    \centering
    \includegraphics[width=0.95\textwidth]{figures/f1_pact_enforcement.png}
    \caption{End-to-end \system{} enforcement path. PACT checks role-labelled
arguments, provenance, registered transformations, and sinks before
\texttt{ToolExecutor}; denied calls create no tool event or side effect.}
    \label{fig:architecture}
\end{figure*}
...
\subsection{Security Invariants and Evidence-to-Claim Mapping}

PACT is best understood as a reference-monitor policy over structured tool
arguments, not as a claim that the language model never follows an injected
instruction. For a proposed call with tool $t$ and arguments $a_i$, the gateway
checks whether the task manifest authorizes the tool and values, whether each
argument's provenance meets the authority required by its role, and whether any
derived value exactly matches a registered source, output, and transformation.
All applicable checks must hold before execution, so the protected property does
not depend on the model recognizing or refusing the attack text.

The first invariant is task-scoped least authority. Every executed call must
satisfy tool, resource, recipient, participant, parameter-range, and call-count
constraints. This explains why capability-enabled defenses stop the observed
T1--T4 violations, while preserving the explicit assumption that the manifest
itself is a correct authorization oracle.

The second invariant is that data validity does not imply authority. Bob is a
valid allow-listed recipient in the deterministic case, so Capability-Only
accepts him even when the value comes from external email. PACT rejects that
binding because external provenance cannot authorize the high-impact
\texttt{recipient} role. Likewise, a recognized transformation name cannot
upgrade source authority: only an exact registered transformation of a trusted
source preserves the authority required by the destination role.

The third invariant is selective usability rather than blanket distrust.
External text remains usable in the low-risk \texttt{content} role when the
recipient is user-authorized. A denied high-impact argument is stopped before
\texttt{ToolExecutor}, leaving no tool event or outbox side effect. Together,
these cases test both authority-laundering resistance and preservation of benign
user selection and external-content handling.

The evidence layers support different claims. Six tool-boundary records connect
decisions to actual synthetic side effects; the eight-row matrix isolates role
and transformation combinations; the factorial experiment exercises
model-generated proposals and exposes clean-task cost; and the sink-pressure
test covers multiple outbound fields. AgentDojo supplies broader attack realism
but is not treated as a PACT evaluation, so its denominators and metric
semantics remain separate.

These invariants hold only inside the stated trusted boundary. They do not cover
forged provenance labels, an incorrect manifest, unmediated sinks, a compromised
gateway, or semantic transformations missed by exact matching. The current
matrix also declares \texttt{target} as high impact without instantiating it in
a separate row; keeping this distinction explicit aligns the claim with the
released evidence. The claim is therefore deterministic enforcement for
recorded structured calls, not general noninterference or universal
...
The evidence suggests a deployable division of responsibility between the agent
and the gateway. The agent may retrieve external data and propose a structured
action, but the gateway should evaluate the capability manifest and the
provenance of each argument before invoking a side effect. High-impact roles
such as \texttt{recipient}, \texttt{target}, and \texttt{control} should fail
closed when their source authority is insufficient, while externally supplied
text may still enter a low-risk \texttt{content} field. Each decision should
retain the role, source digest, registered transformation, policy result, and
world-state outcome so that an operator can replay the decision rather than
infer it from the final message.

The prototype also makes its future-work boundary explicit. The current
exact-match registry does not establish semantic equivalence for paraphrases,
encoding, splitting, or cross-tool transformations; extending it requires
typed transformation contracts and independent validation rather than fuzzy
matching. Adaptive red agents, multiple models and seeds, and larger public
benchmark transfer studies are needed to estimate robustness beyond the frozen
six-task corpus. User confirmation, secret-broker mediation, and side-channel
isolation should be evaluated as separate layers. These extensions should
preserve the present metric separation: authority and permission violations in
T1--T4 versus sensitive-value leakage in T5--T6.
